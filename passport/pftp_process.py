...
class PFTPProcess(Process):
    ...
    def task_analyze_receipt_upload_to_mdn(self):
        ...
        # others 中的回执
        client_npts_tmp_files = os.listdir(settings.NPTS_RECEIPT_INOBX)
        client_npts_tmp_files = self.handle_files_order_tatm(client_npts_tmp_files, settings.NPTS_RECEIPT_INOBX)
        client_nems_tmp_files = os.listdir(settings.NEMS_RECEIPT_INOBX)
        client_nems_tmp_files = self.handle_files_order_tatm(client_nems_tmp_files, settings.NEMS_RECEIPT_INOBX)
        client_tatm_tmp_files = client_npts_tmp_files + client_nems_tmp_files
        logger.info("加贸需要处理的文件个数:{}".format(len(client_tatm_tmp_files)))

        if len(client_tatm_tmp_files) > 0:

            for name in client_tatm_tmp_files:
                file_path = os.path.join(settings.NPTS_RECEIPT_INOBX, name)
                handler = ReceiptHandler(file_path)
                handler.exec()

            logger.info("分析others回执,生成回执上传完成")

    def handle_files_order_tatm(self, files, files_path):
    ...
    def handle_files_order(self,files):
        ...
        for file_name in files:
            if file_name.startswith("Receipt"):
                receipt_files.append(file_name)
            elif file_name.startswith(("Successed", 'Failed')):
                other_files.append(file_name)
