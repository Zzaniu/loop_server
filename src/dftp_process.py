...
from src.online import OnlineReceiptHandler
...
class DFTPProcess(Process):
    ...
    def analyze_receipt_upload_to_mdn(self):
        ...
        t.start()
        # 这里是一线业务的
        # t1 = Thread(target=self.thread_analyze_receipt_upload_to_mdn_online)
        # t1.start()

    def thread_analyze_receipt_upload_to_mdn_online(self):
        """分析加贸回执,生成回执上传到mdn文件夹的线程"""
        while 1:
            try:
                logger.info('ftp analyze upload start')
                self.task_analyze_receipt_upload_to_mdn_online()
    ...
    def task_analyze_receipt_upload_to_mdn_online(self):
        """分析回执,更新数据库状态"""
        client_tmp_files = os.listdir(settings.RECEIPT_INOBX_NEMS)
        client_tmp_files.extend(os.listdir(settings.RECEIPT_INOBX_NTPS))
        logger.info("需要处理的文件个数:{}".format(len(client_tmp_files)))

        client_tmp_files = self.handle_files_order_tatm(client_tmp_files)

        if len(client_tmp_files) > 0:

            for name in client_tmp_files:
                file_path = os.path.join(settings.RECEIPT_INOBX, name)
                handler = OnlineReceiptHandler(file_path)
                handler.exec()

            logger.info("分析回执,生成回执上传完成")

    def handle_files_order_tatm(self, files):
        """将加贸回执按照生成时间dDate排序，需要打开文件获取内容"""

        receipt_files = []
        other_files = []

        for file_name in files:
            if file_name.startswith("Receipt"):
                receipt_files.append(file_name)
            elif file_name.startswith("Successed"):
                other_files.append(file_name)

        receipt_files_dict = {}
        for file_name in receipt_files:
            file_path = os.path.join(settings.NPTS_RECEIPT_INOBX, file_name)
            if os.path.exists(file_path):
                pass
            else:
                file_path = os.path.join(settings.NEMS_RECEIPT_INOBX, file_name)
            with open(file_path, encoding="utf-8") as f:
                content = f.read()
                if file_name.startswith('Receipt_C'):
                    ret = re.search(r"<manageDate>(.*?)</manageDate>", content)  # 按照通知时间将回执排序
                elif file_name.startswith('Receipt_D'):
                    ret = re.search(r"<createDate>(.*?)</createDate>", content)  # 按照通知时间将回执排序
                else:
                    pass
                if ret:
                    dDate = ret.group(1)
    ...
    def task_analyze_receipt_upload_to_mdn(self):
        """分析回执,更新数据库状态"""
        client_tmp_files = os.listdir(settings.RECEIPT_INOBX)
        logger.info("需要处理的文件个数:{}".format(len(client_tmp_files)))
    ...
    def handle_files_order(self,files):
        ...
        receipt_files_dict = {}
        for file_name in receipt_files:
            file_path = os.path.join(settings.RECEIPT_INOBX, file_name)
            with open(file_path, encoding="utf-8") as f:

                content = f.read()
                ret = re.search(r"<NOTICE_DATE>(.*?)</NOTICE_DATE>", content)#按照通知时间将回执排序
                if ret:
                    dDate = ret.group(1)
                    # 去掉回执XML时间栏中的 T
