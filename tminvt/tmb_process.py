...
from tminvt import tminvt

logger = log.getlogger(__name__)


class TmBProcess(Process):
    def __init__(self):
        super().__init__()
    ...
    def exec_task(self):
        _Sql = sql.Sql()
        dec_infos = _Sql.select("TMMsg", "id", "ClientSeqNo", "DecState", "CreateUser",
                                where={"DecState": 'TS_RDY', "DeleteFlag": "0"})

        logger.info('TMMsg = {}'.format(dec_infos))
    ...
    def upload_QP_task(self, dec_info):
        ...
        dec_head_fields = ["id", "BONDINVTNO", "SEQNO", "PUTRECNO", "ETPSINNERINVTNO", "RLTENTRYBIZOPETPSSCCD",
                           ...
                           "STSHIPTRSARVNATCD", "BONDINVTTYPECD", "Rmk", "TMId", "DelcareFlag", 'BIZOPETPSNO',
                           'BIZOPETPSSCCD', 'BIZOPETPSNM']

        dec_head_info = _Sql.select("TradeManualInvtHeadType", *dec_head_fields, where={"TMId": dec_id})
        ...
        dec_list_fields = ["id", "SEQNO", "GDSSEQNO", "PUTRECSEQNO", "GDSMTNO", "GDECD", "GDSNM", "GDSSPCFMODELDESC",
                           ...
                           "FLOWAPPLYTBSEQNO", "APPLYTBSEQNO", "Rmk", "TMId"]
        dec_lists_info = _Sql.select("TradeManualInvtListType", *dec_list_fields,
                                     where={"TMId": dec_id, 'DeleteFlag': 0})
        dec_list_list = []
        ...
        import os
        xml_file_name = os.path.join(settings.GOLD_XML_DIR, "TMInvt" + ClientSeqNo + ".xml")

        _xml = tminvt.Xml(xml_file_name, dec)  # 将dec传入xml然后保存到磁盘先,单一窗口类型

        _xml.save()
        from conf.zipfiles import zip_files
        zip_files(xml_file_name)
        logger.info('生成加贸手册核注清单ZIP报文，自编号{}'.format(ClientSeqNo))
        if settings.DEBUG:
            d = {
                "DecState": 'TS_REQ',
            }
            # 更新状态为“草单，申报提交”
            _Sql.update("TMMsg", where={"ClientSeqNo": ClientSeqNo, 'DeleteFlag': 0}, **d)
            logger.info('更新TMMsg状态为TS_REQ，自编号{}'.format(ClientSeqNo))


if __name__ == "__main__":
    tm_db = TmBProcess()
    tm_db.start()  # start会自动调用run
